...
def build_maskrcnn(backbone, num_classes, min_size, max_size):
    """
    ...
    mask_head = MaskRCNNHeads(backbone.out_channels, [256, 256, 256, 256], 1, norm_layer=partial(nn.GroupNorm, 32))

    return MaskRCNN(
        backbone, num_classes, min_size, max_size,
        rpn_anchor_generator=rpn_anchor_generator, rpn_head=rpn_head, box_head=box_head, mask_head=mask_head,
    )
