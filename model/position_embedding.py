...
class PositionEmbedding2d(nn.Module):
    ...
    def forward(self, x):
        ...
        pos_embedding = get_2d_pos_embedding(d_model, height, width)  # (d_model, height, width)
...
class RelativePositionEmbedding2d(nn.Module):
    ...
    def forward(self, x):
        ...
        return relative_pos.to(device)


def get_1d_pos_embedding(d_model, seq_len):
    """
...
def get_2d_pos_embedding(d_model, height, width):
    """
    :param d_model: The hidden dimension.
    :param height: The height of the image.
    :param width: The width of the image.
    """
    embed_h = get_1d_pos_embedding(d_model // 2, height)  # (d_model/2, height)
    embed_w = get_1d_pos_embedding(d_model // 2, width)  # (d_model/2, width)
